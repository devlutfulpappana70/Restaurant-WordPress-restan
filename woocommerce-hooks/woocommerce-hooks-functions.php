...
// restan shop main content hook functions
if( !function_exists('restan_shop_main_content_cb') ) {
    function restan_shop_main_content_cb( ) {

        if( is_shop() || is_product_category() || is_product_tag() ) {
            echo '<div class="validtheme-shop-area default-padding bottom-less">';
...
// restan shop main content hook function
if( !function_exists('restan_shop_main_content_end_cb') ) {
    function restan_shop_main_content_end_cb( ) {
...
if( ! function_exists('restan_woocommerce_filter_wrapper') ) {
    function restan_woocommerce_filter_wrapper( ) {
...
if( ! function_exists('restan_woocommerce_tab_content_wrapper_start') ) {
    function restan_woocommerce_tab_content_wrapper_start( ) {
        echo '<!-- Tab Content -->';
...
// woocommerce tab content wrapper start hook function
if( ! function_exists('restan_woocommerce_tab_content_wrapper_end') ) {
    function restan_woocommerce_tab_content_wrapper_end( ) {
        echo '</div></div></div>';
        echo '<!-- End Tab Content -->';
    }
}

// restan grid tab content hook function
if( !function_exists('restan_grid_tab_content_cb') ) {
    function restan_grid_tab_content_cb( ) {
...
// restan list tab content hook function
if( !function_exists('restan_list_tab_content_cb') ) {
    function restan_list_tab_content_cb( ) {
        echo '<!-- List -->';
...
// restan loop horizontal product thumbnail hook function
if( !function_exists('restan_loop_horiontal_product_thumbnail') ) {
    function restan_loop_horiontal_product_thumbnail( ) {
                ...
                echo '<div class="product-image">';
                    ...
                    $sale_price     = get_post_meta( $product->get_id(), '_sale_price', true );
                     ...
                     if( !empty($sale_price) ) {
                        ...
                        echo "<span class='onsale'>" . number_format($percentage,0, '', '') .esc_html__('% Off', 'restan'). "</span>";
                    }
                }
                ...
                echo '<div class="product-caption">';
                    ...
                    echo sprintf(
                        ...
                        esc_attr(implode(' ', array_filter(array(
                            ...
                            'cart-btn ajax_add_to_cart add_to_cart_button'
                        )))),
                        '<i class="fas fa-shopping-bag"></i>'.esc_html($product->add_to_cart_text()).'',
                        
                    );

                    }else{
                        echo '<a href="'.$product->add_to_cart_url().'"  class="cart-btn ajax_add_to_cart add_to_cart_button" data-product_id="'.get_the_ID().'" data-product_sku="'.esc_attr($product->get_sku()).'" >';
...
// restan loop product thumbnail hook function
if( !function_exists('restan_loop_product_thumbnail') ) {
    function restan_loop_product_thumbnail( ) {
        global $product;
        echo '<div class="product-contents">';
            echo '<div class="product-image">';
                if( $product->is_type('simple') || $product->is_type('external') || $product->is_type('grouped') ) {
...
function woocommerce_template_loop_add_to_cart( $args = array() ) {
    global $product;
        ...
        echo sprintf( '<a href="%s" data-quantity="%s" class="%s" %s>%s</a>',
            ...
            '<i class="fas fa-shopping-cart"></i> <span>'.esc_html__('Add to cart', 'restan').'</span>'
        );
}

// before single product summary hook
if( ! function_exists('restan_woocommerce_before_single_product_summary') ) {
    function restan_woocommerce_before_single_product_summary( ) {
        global $post,$product;
...
if( ! function_exists('restan_woocommerce_template_single_rating_and_tags') ) {
    function restan_woocommerce_template_single_rating_and_tags( ) {
        global $product;
...
if( ! function_exists('restan_woocommerce_single_product_title') ) {
    function restan_woocommerce_single_product_title( ) {
        echo '<h2 class="product-title">'.esc_html( get_the_title() ).'</h2>';
    }
}

// woocommerce single products price
if( ! function_exists('restan_woocommerce_single_product_price') ) {
    function restan_woocommerce_single_product_price( ) {
        echo '<div class="price">';
            echo woocommerce_template_loop_price();
        echo '</div>';
...
if( !function_exists('restan_woocommerce_single_product_availability') ) {
    function restan_woocommerce_single_product_availability( ) {
        global $product;
        ...
        if( $availability['class'] != 'out-of-stock' ) {
            if( $product->get_stock_quantity() ){
                echo '<div class="product-stock validthemes-in-stock">';
                    echo '<div class="product-quantity">'.esc_html( $product->get_stock_quantity() ).' '.esc_html__( ' items availabe', 'restan' ).'</div>';
                echo '</div>';
            }else{
                echo '<div class="product-stock validthemes-in-stock">';
                echo '<span>'.esc_html__( 'In Stock', 'restan' ).'</span>';
            echo '</div>';
            }
            
        }else{
            echo '<div class="product-stock validthemes-in-stock">';
                echo '<span>'.esc_html__( 'Out Of Stock', 'restan' ).'</span>';
            echo '</div>';
        }
    }
}
...
if( !function_exists('restan_woocommerce_single_product_excerpt') ) {
    function restan_woocommerce_single_product_excerpt( ) {
        woocommerce_template_single_excerpt();
    }
}

// single product add to cart fuunction
if( !function_exists('restan_woocommerce_single_add_to_cart_button') ) {
    function restan_woocommerce_single_add_to_cart_button( ) {
        woocommerce_template_single_add_to_cart();    
    }
}

// single product ,eta hook function
if( !function_exists('restan_woocommerce_single_meta') ) {
    function restan_woocommerce_single_meta( ) {
        global $product;

        echo '<div class="product-meta">';
            if( ! empty( $product->get_sku() ) ){
                echo '<span class="sku">';
                    echo '<strong>'.esc_html__( 'SKU:', 'restan' ).'</strong> '.$product->get_sku().'';
                echo '</span>';
            }
            echo wc_get_product_category_list( $product->get_id(), ', ', '<span class="posted_in">' . _n('<strong>'.__("Category:",'restan').'</strong>', 'Categories:', count( $product->get_category_ids() ), 'restan' ) .' ', '</span>' );
        echo '</div>';
    }
}
...
if( !function_exists('woocom_shipping_product_delivery_duration') ) {
    ...
    global $woocommerce, $post;
        ...
        woocommerce_wp_text_input( 
            array( 
                'id'                => '_restan_shipping_durations', 
                'label'             => __( 'Dalivery Time (in days)', 'restan' ), 
                'desc_tip'          => 'true',
                'description'       => __( 'How many days you need to deliver this product?', 'restan' ),
            )
        );
        woocommerce_wp_text_input( 
            array( 
              'id'              => '_delivery_text', 
              'label'           => __( 'Delivery Text', 'restan' ), 
              'placeholder'     => __('Speedy and reliable parcel delivery!', 'restan'),
              'desc_tip'        => 'true',
              'description' => __( 'This text will shown on sigle product page after the delivery date text', 'restan' ) 
            )
        );
    }
}
...
if( !function_exists('woocom_save_general_proddata_custom_field') ) {
    ...
    function woocom_save_general_proddata_custom_field( $post_id ) {
        ...
        // days
        $restan_shipping_durations = $_POST['_restan_shipping_durations'];
        if( ! empty( $restan_shipping_durations ) ) {
            update_post_meta( $post_id, '_restan_shipping_durations', esc_attr( $restan_shipping_durations ) );
        }
...
// estimate delivery
if( !function_exists('restan_woocommerce_product_estimate_delivary') ) {
    function restan_woocommerce_product_estimate_delivary( ) {
        global $product;
        $delivery_durations = get_post_meta( get_the_ID(), '_restan_shipping_durations', true );
        ...
        if(!empty($delivery_durations)){
            ...
            echo '<div class="product-estimate-delivary"><i class="fas fa-box-open"></i>';
                ...
                echo '<strong> '.esc_html( $delivery_durations ).''.esc_html__(' day Delivery', 'restan').'</strong>';
                $note = !empty($day_duration_text) ? $day_duration_text : __( 'Speedy and reliable parcel delivery!', 'restan' );
...
add_action( 'woocommerce_after_quantity_input_field', 'restan_display_quantity_plus' );
  
function restan_display_quantity_plus() {
   echo '<button type="button" class="plus">+</button>';
}
  
add_action( 'woocommerce_before_quantity_input_field', 'restan_display_quantity_minus' );
  
function restan_display_quantity_minus() {
...
add_action( 'wp_footer', 'restan_add_cart_quantity_plus_minus' );
  
function restan_add_cart_quantity_plus_minus() {
...
add_action( 'woosq_product_summary', 'restan_woosq_template_single_rating', 10 );

function restan_woosq_template_single_rating() {
    global $product;
    $review_count = $product->get_review_count();
    if ( $review_count > 0 ) : 
        echo '<div class="review-count">';
...
// restan woocommerce get sidebar hook function
if( ! function_exists('restan_woocommerce_get_sidebar') ) {
    function restan_woocommerce_get_sidebar( ) {
        echo '<div class="col-lg-3 shop-left-sidebar">';
            dynamic_sidebar( 'restan-shop-sidebar' );
        echo '</div>';
    }
}
...
function woocommerce_product_custom_fields() {
    ...
    woocommerce_wp_text_input( array( 
        'id'          => 'tipologiaAppunto',
        'label'       => __('Tipologia appunto:', 'restan'),
        'placeholder' => '',
